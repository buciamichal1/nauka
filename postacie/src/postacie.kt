...
open class Mag(imie:String,postac: Postac) : Postac(imie) {
    ...
    fun atak_mag(postac: Postac, wojownik: Wojownik,crit: Float) {
        if(mana >= 20){
            ...
            mana -= 20
            println("Zadałeś obrazenia magiczne czarem przeciwnikowi ${wojownik.imie} równe ${damage_mag_czarem}")
    ...
    fun atak_fiz(postac: Postac, wojownik: Wojownik,crit: Float) {
        ...
        println("Zadałeś obrazenia fizyczne kosturem przeciwnikowi ${wojownik.imie} równe ${damage_fiz_kosturem}")
        println("${wojownik.imie} hp ${wojownik.zycie}")
    }
}
open class Wojownik(imie:String,postac: Postac) : Postac(imie) {
    ...
    fun atak_mieczem(postac: Postac, mag: Mag,crit: Float) {
        if(stamina >= 15){
            ...
            stamina -= 15
            println("Zadałeś obrazenia fizyczne mieczem przeciwnikowi ${mag.imie} równe ${damage_fiz_mieczem}")
    ...
    fun atak_tarcza(postac: Postac, mag: Mag,crit: Float) {
        if(stamina >= 10){
            ...
            stamina -= 10
            println("Zadałeś obrazenia fizyczne tarczą przeciwnikowi ${mag.imie} równe ${damage_fiz_tarcza}")
            println("${mag.imie} hp ${mag.zycie}")
...
fun main() {
    //var randomInt = (0..1).random()
    var tomusPostac = Postac("Tomuś")
    var pawelekPostac = Postac("Pawełek")
    var tomus = Mag("Tomuś", tomusPostac)
    var pawelek = Wojownik("Pawełek", pawelekPostac)
    start(tomus,pawelek)
    while (true) {
        ...
        when (keyCode){
            /*'\t'.toInt() -> {val key = System.`in`.read()
                                when(key){*/
                                    'w'.toInt() -> {tomus.atak_mag(tomus, pawelek,crit)}
                                    'd'.toInt() -> {tomus.atak_fiz(tomus, pawelek,crit)}
                                    /*else -> println("miss")
                                }}
            '+'.toInt() -> {val key = System.`in`.read()
                                when(key){*/
                                    '8'.toInt() -> {pawelek.atak_mieczem(pawelek, tomus,crit)}
                                    '4'.toInt() -> {pawelek.atak_tarcza(pawelek, tomus,crit)}
                                    /*else -> println("miss")
                                }}*/
            10 -> {statystyki(tomus,pawelek)}
            else -> {println("Zły klawisz")}
        }
        if(pawelek.zycie < 1){println("Wygrał Tomuś")
            break}
        if(tomus.zycie < 1){println("Wygrał Pawełek")
            break}
